...
可用设备：core-sw-1（核心交换机）, core-rtr-1（核心路由器）, fw-1（防火墙）
...
_KNOWN_TOOL_NAMES = {
    "run_device_command", "ping", "traceroute", "search_runbook",
...
async def run_agent(message: str, history: list[dict], role: str = "operator") -> AsyncIterator[dict]:
    ...
    def _evidence_hint() -> str:
        ...
        if checked_interface and not (checked_acl or checked_arp):
            return (
    ...
    try:
        while steps < MAX_STEPS:
            ...
            try:
                ...
                ok = True
                if parsed.name in ("run_device_command", "ping", "traceroute") and args.get("device"):
